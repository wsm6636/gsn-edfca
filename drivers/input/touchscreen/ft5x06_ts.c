...
/*
	...
	unsigned int reqirq_id=IRQ_EINT(1);//
#elif defined(CONFIG_OK210_BOARD_V1)||defined(CONFIG_OK210_BOARD_V3)
	unsigned int reqirq_id=IRQ_EINT(27);
#endif
*/
unsigned int reqirq_id ;
//register address
...
int tsp_keycodes_forlinx[CFG_NUMOFKEYS_FORLINX] ={
        KEY_BACK,
        KEY_HOME,
        KEY_MENU
};
...
char *tsp_keyname_forlinx[CFG_NUMOFKEYS_FORLINX] ={
...
static bool tsp_keystatus_forlinx[CFG_NUMOFKEYS_FORLINX];
...
static void ft5x0x_report_value(void)
{
...
#else   //linux
        if (event->au16_x[0] < TOUCHSCREEN_MAXX && event->au16_y[0] < TOUCHSCREEN_MAXY)  // LCD view area
        {
            if (event->au16_x[0]!= 0 && event->au16_y[0]!= 0)
            {
                input_report_abs(data->input_dev, ABS_X, event->au16_x[0]);
                input_report_abs(data->input_dev, ABS_Y, event->au16_y[0]);
                input_report_abs(data->input_dev, ABS_Z, 0);
            }

            input_report_key(data->input_dev, BTN_TOUCH, 1);
            input_report_abs(data->input_dev, ABS_PRESSURE, 1);
            //printk("Linux Touch down,it is start \n");
    ...
    if(event->touch_point==0)//touch up
    {
     ...
     //printk("Linux Touch up,it is end \n");
...
***********************************************************************************************/
static struct proc_dir_entry *ts_proc_entry=0; //forlinx
static int  ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   // s3c_setup_capacitouch_cfg_gpio(); //forlinx add 2012-10-15

    err = request_irq(reqirq_id, ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING, "ft5x0x_ts", ft5x0x_ts);
	if (err < 0) {
        dev_err(&client->dev, "ft5x0x_probe: request irq failed %s:%d \n ",__FILE__,__LINE__);
...
static const struct of_device_id ft5x0x_ts_dt_ids[] = {
...
static struct i2c_driver ft5x0x_ts_driver = {
	...
	.driver	= {
		.name	= FT5X0X_NAME,
		.owner	= THIS_MODULE,
		.of_match_table	= ft5x0x_ts_dt_ids,
	},
};
