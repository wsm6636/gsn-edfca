...
static inline int check_for_preemptions_helper(void)
{
	...
	cpu_to_preempt = this_cpu_ptr(&gsnnpfpca_cpu_entries);
 ...
 */
void gsnnpfpca_check_sched_invariant(void)
{
	int i;
	cpu_entry_t *entry = NULL;
	...
	entry = this_cpu_ptr(&gsnnpfpca_cpu_entries);
	task = entry->linked;
...
{
	rt_domain_t *rt = &gsnnpfpca;
	cpu_entry_t* entry = this_cpu_ptr(&gsnnpfpca_cpu_entries);
    ...
    if (is_realtime(current))
    {
        if (tsk_rt(current)->job_params.cache_state & (CACHE_WILL_USE | CACHE_IN_USE))
        {
            ...
            selective_flush_cache_partitions(entry->cpu,
                tsk_rt(current)->job_params.cache_partitions, current, &gsnnpfpca);
...
static void set_priority_inheritance(struct task_struct* t, struct task_struct* prio_inh)
{
	...
	if (linked_on != NO_CPU) {
		...
		bheap_delete(cpu_lower_prio, &gsnnpfpca_cpu_heap,
			    gsnnpfpca_cpus[linked_on]->hn);
		bheap_insert(cpu_lower_prio, &gsnnpfpca_cpu_heap,
			    gsnnpfpca_cpus[linked_on]->hn);
	} else {
		/* holder may be queued: first stop queue changes */
		raw_spin_lock(&gsnnpfpca.release_lock);
		if (is_queued(t)) {
		...
		raw_spin_unlock(&gsnnpfpca.release_lock);
...
struct task_struct* find_hp_waiter_npfpca(struct fmlp_semaphore *sem,
...
int gsnnpfpca_fmlp_lock(struct litmus_lock* l)
{
...
int gsnnpfpca_fmlp_unlock(struct litmus_lock* l)
{
	...
	if (next) {
		...
		if (next == sem->hp_waiter) {
			 ...
			 * hp_waiter. */
			sem->hp_waiter = find_hp_waiter_npfpca(sem, next);
			if (sem->hp_waiter)
...
int gsnnpfpca_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		gsnnpfpca_fmlp_unlock(l);

	return 0;
}

void gsnnpfpca_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops gsnnpfpca_fmlp_lock_ops = {
	.close  = gsnnpfpca_fmlp_close,
	.lock   = gsnnpfpca_fmlp_lock,
	.unlock = gsnnpfpca_fmlp_unlock,
	.deallocate = gsnnpfpca_fmlp_free,
};

static struct litmus_lock* gsnnpfpca_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &gsnnpfpca_fmlp_lock_ops;
...
static long gsnnpfpca_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user unused)
{
	int err = -ENXIO;
