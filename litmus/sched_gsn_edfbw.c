...
static int check_for_bw_preemptions(cpu_entry_t *entry, struct task_struct *task)
{
	...
	num_used_bw_partitions = bw_count_set_bits(rt->used_bw_partitions & BANDWIDTH_PARTITIONS_MASK);
...
void gsnedfbw_check_sched_invariant(void)
{
	...
	num_used_bw = bw_count_set_bits(rt->used_bw_partitions & BANDWIDTH_PARTITIONS_MASK);
