...
static struct fb_videomode lcdif_modedb[] = {
	...
	0,},
        {
        /* 1024x600 @ 60 Hz , pixel clk @ 33.3MHz */
        "CLAA-WVGA-HD", 60, 1024, 600, 19351, 160, 150, 23, 11, 10, 1,
        FB_SYNC_CLK_LAT_FALL,
        FB_VMODE_NONINTERLACED,
        0,},
	{
	...
	0,},
	{
        /* 480x272 @ 60 Hz  ,pix clk 9MHz */
        "NHD-4.3-ATXI", 60, 480, 272, 111111, 2, 2, 1, 1, 41, 10,
        FB_SYNC_CLK_LAT_FALL,
        FB_VMODE_NONINTERLACED,
        0,},
};
...
static int mxc_lcdif_probe(struct platform_device *pdev)
{
	...
	struct mxc_lcd_platform_data *plat_data;
	struct regulator *lcdif_power_on;
	...
	lcdif_power_on = devm_regulator_get(&pdev->dev, "disp-power-on");
	if(!IS_ERR(lcdif_power_on)){
			regulator_enable(lcdif_power_on);
	} else 
		dev_info(&pdev->dev,"lcdif failed to enabled display power regulator\n");
