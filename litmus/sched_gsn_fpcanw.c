...
#define gsnfpcanw_cache_lock (gsnfpcanw.cache_lock)
...
static inline int check_for_preemptions_helper(void)
{
	...
	cpu_to_preempt = this_cpu_ptr(&gsnfpcanw_cpu_entries);
 ...
 */
void gsnfpcanw_check_sched_invariant(void)
{
	int i;
	cpu_entry_t *entry = NULL;
	...
	entry = this_cpu_ptr(&gsnfpcanw_cpu_entries);
	task = entry->linked;
...
{
	rt_domain_t *rt = &gsnfpcanw;
	cpu_entry_t* entry = this_cpu_ptr(&gsnfpcanw_cpu_entries);
...
static void set_priority_inheritance(struct task_struct* t, struct task_struct* prio_inh)
{
	...
	if (linked_on != NO_CPU) {
		...
		bheap_delete(cpu_lower_prio, &gsnfpcanw_cpu_heap,
			    gsnfpcanw_cpus[linked_on]->hn);
		bheap_insert(cpu_lower_prio, &gsnfpcanw_cpu_heap,
			    gsnfpcanw_cpus[linked_on]->hn);
	} else {
		/* holder may be queued: first stop queue changes */
		raw_spin_lock(&gsnfpcanw.release_lock);
		if (is_queued(t)) {
		...
		raw_spin_unlock(&gsnfpcanw.release_lock);
...
struct task_struct* find_hp_waiter_fpcanw(struct fmlp_semaphore *sem,
...
int gsnfpcanw_fmlp_lock(struct litmus_lock* l)
{
...
int gsnfpcanw_fmlp_unlock(struct litmus_lock* l)
{
	...
	if (next) {
		...
		if (next == sem->hp_waiter) {
			 ...
			 * hp_waiter. */
			sem->hp_waiter = find_hp_waiter_fpcanw(sem, next);
			if (sem->hp_waiter)
...
int gsnfpcanw_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
	...
	if (owner)
		gsnfpcanw_fmlp_unlock(l);

	return 0;
}

void gsnfpcanw_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops gsnfpcanw_fmlp_lock_ops = {
	.close  = gsnfpcanw_fmlp_close,
	.lock   = gsnfpcanw_fmlp_lock,
	.unlock = gsnfpcanw_fmlp_unlock,
	.deallocate = gsnfpcanw_fmlp_free,
};

static struct litmus_lock* gsnfpcanw_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &gsnfpcanw_fmlp_lock_ops;
...
static long gsnfpcanw_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user unused)
{
	int err = -ENXIO;
