//======================================================================
// Driver for the Epson RTC module RX8010 SJ
// 
...
//	ctrl[0] |= 0x02;						//set TSEL for 1Hz 
	ctrl[0] &= ~RX8010_BIT_EXT_TSEL;                //set to 0 (off) for this case
	ctrl[0] |= RX8010_BIT_EXT_USEL;         //set UIE 1 min
	ctrl[0] |= 0x04;
...
//		*need_reset = 1;
//	}
		if(!(ram & 0x01))       //if ram0 bit 1 is 0
			*need_reset = 1;
		else
			rx8010_write_reg(client, RX8010_REG_FLAG, 0x00);

	} else if(!(ram & 0x01))
		err = rx8010_write_reg(client, 0x20, 0x01);
...
static int rx8010_set_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	...
	if (rx8010->ctrlreg & (RX8010_BIT_CTRL_AIE | RX8010_BIT_CTRL_UIE)) {
		rx8010->ctrlreg &= ~(RX8010_BIT_CTRL_AIE | RX8010_BIT_CTRL_UIE);
	...
	//clear Alarm Flag
	flagreg &= ~(RX8010_BIT_FLAG_AF | RX8010_BIT_FLAG_UF);
...
//			ctrl |= (RX8010_BIT_CTRL_AIE | RX8010_BIT_CTRL_UIE);		//set alarm interrupt enable
			ctrl |= (RX8010_BIT_CTRL_AIE );         //set alarm interrupt enable
	}
	else
	{
		if ( ! rx8010->rtc->uie_rtctimer.enabled )
...
#ifdef CONFIG_PM
static int rx8010_suspend(struct device *dev)
{
...
static int rx8010_resume(struct device *dev)
{
       struct i2c_client *client = to_i2c_client(dev);
        if (device_may_wakeup(dev))
...
static SIMPLE_DEV_PM_OPS(rx8010_pm_ops, rx8010_suspend, rx8010_resume);
#endif
