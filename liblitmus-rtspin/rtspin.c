...
#define OPTSTR "p:c:wlveo:F:s:m:q:r:X:L:Q:iRu:Bhd:C:b:a:"
int main(int argc, char** argv)
{
	...
	unsigned int cache_partitions=0;
	unsigned int bw_partitions=0;
	progname = argv[0];
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		case 'i':
			...
			break;
		case 'a':
			cache_partitions  = atoi(optarg);
			break;
		case 'b':
			bw_partitions  = atoi(optarg);
			break;
		case ':':
	...
	param.num_cache_partitions = cache_partitions;
	param.num_bw_partitions = bw_partitions;
